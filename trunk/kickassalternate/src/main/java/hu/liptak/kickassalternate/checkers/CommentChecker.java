		...
		try
		{
			if (minLength < 0 && pattern == null)
				throw new CheckerException(
						"At least one of 'minLength' and 'pattern' parameters must be set!");
			final String commitComment = hookInstance.getSvnClient().doGetLog(
					hookInstance.getRepositoryRootFile(),
					hookInstance.getTransactionId());
			if (minLength > 0 && commitComment.length() < minLength)
			{
				sb.append("Commit comment length cannot be shorter than ")
						.append(minLength).append("! Your comment was: \"")
						.append(commitComment).append("\".\r\n");
			}
			if (pattern != null && !commitComment.matches(pattern))
			{
				sb.append("Commit comment do not match regular expression '")
						.append(pattern).append("'! Your comment was: \"")
						.append(commitComment).append("\".\r\n");
			}
		}
